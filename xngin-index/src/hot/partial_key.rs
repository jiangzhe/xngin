use std::arch::x86_64::{_pext_u32, _mm256_loadu_si256,
...
impl PartialKey for u8 {
    ...
    #[inline]
    fn kind(single: bool) -> NodeKind {
        if single { NodeKind::SP8 } else { NodeKind::MP8 }
    }
}
impl PartialKey for u16 {
    ...
    #[inline]
    fn kind(single: bool) -> NodeKind {
        if single { NodeKind::SP16 } else { NodeKind::MP16 }
    }
}
impl PartialKey for u32 {
    ...
    #[inline]
    fn kind(single: bool) -> NodeKind {
        if single { NodeKind::SP32 } else { NodeKind::MP32 }
    }
}
...
mod tests {
    ...
    fn test_movemask_u16() {
        unsafe {
            let rs = vec![
                0xffffu16, 0, 0xffff, 0xffff, 0, 0, 0xffff, 0,
                0, 0, 0, 0xffff, 0, 0, 0xffff, 0xffff,
            ];
