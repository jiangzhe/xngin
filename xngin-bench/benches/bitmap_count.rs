...
use xngin_compute::bitmap::{AppendBitmap, ReadBitmap, VecBitmap};

fn bench_count(c: &mut Criterion) {
    const N1: usize = 10240;
    const N2: usize = 10238;
    const N3: usize = 10242;
    let mut bm1 = VecBitmap::with_capacity(N1);
    let mut bm2 = VecBitmap::with_capacity(N2);
    let mut bm3 = VecBitmap::with_capacity(N3);

    let mut thd_rng = thread_rng();
    for _ in 0..N1 {
        bm1.add(thd_rng.gen()).unwrap();
    }
    for _ in 0..N2 {
        bm2.add(thd_rng.gen()).unwrap();
    }
    for _ in 0..N3 {
        bm3.add(thd_rng.gen()).unwrap();
    }
...
fn count_false(bm: &mut VecBitmap) -> usize {
    bm.false_count()
}
